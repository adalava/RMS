...
class UploadManager(multiprocessing.Process):
    ...
    def delayNextUpload(self, delay=0):
        ...
        with self.next_runtime_lock:
            self.next_runtime = datetime.datetime.now(datetime.UTC) + datetime.timedelta(seconds=delay)
        ...
        while not self.exit.is_set():
            ...
            with self.last_runtime_lock:
                ...
                if self.last_runtime is not None:
                    if (datetime.datetime.now(datetime.UTC) - self.last_runtime).total_seconds() < 15*60:
                        time.sleep(1)
                        continue
            ...
            with self.next_runtime_lock:
                ...
                if self.next_runtime is not None:
                    if (datetime.datetime.now(datetime.UTC) - self.next_runtime).total_seconds() < 0:
                        time.sleep(1)
                        continue

            with self.last_runtime_lock:
                self.last_runtime = datetime.datetime.now(datetime.UTC)
