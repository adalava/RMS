...
class EventMonitor(multiprocessing.Process):
    ...
    def checkEvents(self, ev_con, test_mode = False):
        ...
        for observed_event in unprocessed:
            ...
            if convertGMNTimeToPOSIX(observed_event.dt) + \
                    datetime.timedelta(seconds=int(observed_event.time_tolerance)) > datetime.datetime.now(datetime.UTC):
            ...
            check_time_start = datetime.datetime.now(datetime.UTC)
            ...
            # If trajectory inside the closeradius, then do the upload and mark as processed
                if min_dist < event.close_radius * 1000 and not test_mode:
                    ...
                    check_time_end = datetime.datetime.now(datetime.UTC)
        ...
        next_run = (datetime.datetime.now(datetime.UTC) + datetime.timedelta(minutes=self.check_interval)).replace(microsecond = 0)
        if future_events == 1:
    ...
    def run(self):
        ...
        time.sleep(60)
        last_check_start_time = datetime.datetime.now(datetime.UTC)
        while not self.exit.is_set():
            check_start_time = datetime.datetime.now(datetime.UTC)
            next_check_start_time = (datetime.datetime.now(datetime.UTC) + datetime.timedelta(minutes=self.check_interval))
            ...
            if not isinstance(start_time, bool):
                ...
                time_left_before_start = (start_time - datetime.datetime.now(datetime.UTC))
