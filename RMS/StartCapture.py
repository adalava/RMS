...
def wait(duration, compressor, buffered_capture, video_file):
    ...
    while True:
        ...
        if duration is not None:
            ...
            time_elapsed = (datetime.datetime.now(datetime.UTC) - time_start).total_seconds()
    ...
    if upload_manager is not None: # temporary code, will make the script upload after each capture
        ...
        with upload_manager.next_runtime_lock:
            ...
            if upload_manager.next_runtime is not None:
                ...
                sleep_time = None
                while (datetime.datetime.now(datetime.UTC) - upload_manager.next_runtime).total_seconds() < 0:
                    
                    wait_time = (datetime.datetime.now(datetime.UTC) - upload_manager.next_runtime).total_seconds()
